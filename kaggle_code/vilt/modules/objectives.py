...
def focal_loss(inputs, targets, gamma=2, weight=None, pos_weight=None):
    ce_loss = F.binary_cross_entropy_with_logits(inputs, targets, weight=weight, pos_weight=pos_weight)
    return (1 - torch.exp(-ce_loss)) ** gamma * ce_loss


def compute_modmis_bin(pl_module, batch):  # 针对二分类输出  分类BCE
    phase = "train" if pl_module.training else "val"
    if phase == "train":
        infer = pl_module.infer(batch, mask_text=False, mask_image=False)
    else:
    ...
    loss_name = "modmis_bin"
    ret = {f"{loss_name}_labels": torch.stack(batch['label'])}
    if phase in pl_module.hparams.config["feats_show_phase"]:
        getattr(pl_module, f"{phase}_{loss_name}_feats_show").update(infer["out_feats"], ret[f"{loss_name}_labels"], infer["field_tokens"])
    ret[f"{loss_name}_logits"] = pl_module.modmis_binarizer(infer["out_feats"])
    getattr(pl_module, f"{phase}_{loss_name}_composite").update(ret[f"{loss_name}_logits"], ret[f"{loss_name}_labels"])
    if pl_module.hparams.config["used_loss"] == "bcewl":
        ret[f"{loss_name}_loss"] = F.binary_cross_entropy_with_logits(
            ret[f"{loss_name}_logits"],
            ret[f"{loss_name}_labels"].float(),
            pos_weight=pl_module.bcewl_pos_weight
        )
    elif pl_module.hparams.config["used_loss"] == "focal":
        ret[f"{loss_name}_loss"] = focal_loss(
            ret[f"{loss_name}_logits"],
            ret[f"{loss_name}_labels"].float(),
            gamma=pl_module.hparams.config["focal_gamma"]
        )

    pl_module_scale = getattr(pl_module, f"{phase}_{loss_name}_loss")
    pl_module_scale.update(ret[f"{loss_name}_loss"])
    pl_module.log(f"{loss_name}/{phase}/loss", pl_module_scale.compute())
    return ret


def compute_modmis_cls(pl_module, batch):  # 分类头输出  分类CE
    phase = "train" if pl_module.training else "val"
    if phase == "train":
        infer = pl_module.infer(batch, mask_text=False, mask_image=False)
    else:
    ...
    loss_name = "modmis_cls"
    ret = {f"{loss_name}_labels": torch.stack(batch['label']['field']),}
    ret[f"{loss_name}_logits"] = pl_module.modmis_classifier(infer["out_feats"])
    getattr(pl_module, f"{phase}_{loss_name}_composite").update(ret[f"{loss_name}_logits"], ret[f"{loss_name}_labels"])
    ret[f"{loss_name}_loss"] = torch.sum([
        F.cross_entropy(logit, target)
        for logit, target in zip(ret[f"{loss_name}_logits"], ret[f"{loss_name}_labels"].transpose())
    ])
...
def compute_modmis_reg(pl_module, batch):  # 回归头输出  回归MSE
    phase = "train" if pl_module.training else "val"
    if phase == "train":
        infer = pl_module.infer(batch, mask_text=False, mask_image=False)
    else:
    ...
    loss_name = "modmis_reg"
    ret = {f"{loss_name}_labels": torch.stack(batch['label']['field']).float(),}
    ret[f"{loss_name}_preds"] = pl_module.modmis_regresser(infer["out_feats"])
    getattr(pl_module, f"{phase}_{loss_name}_composite").update(ret[f"{loss_name}_preds"], ret[f"{loss_name}_labels"])
    ret[f"{loss_name}_loss"] = F.mse_loss(ret[f"{loss_name}_preds"], ret[f"{loss_name}_labels"])
