...
from vilt.gadgets.my_metrics import Accuracy, VQAScore, Scalar, F1_Score, AUROC,\
    MM_Composite_Cls, MM_Composite_Reg, MM_Composite_Bin, FeatsShower


def set_metrics(pl_module):
    for split in ["train", "val"]:
        for k, v in pl_module.hparams.config["loss_names"].items():
            if v < 1:  # 判断config中某指标有无设置 <1无 >=1有
                continue
            if k == "modmis_bin":
                version_dir = f"{pl_module.hparams.config['log_dir']}/{pl_module.hparams.config['pl_log_name']}/"
                version_dir += f"version_{max([int(item[8:])+1 for item in os.listdir(version_dir)])}"
                setattr(pl_module, f"{split}_{k}_composite",
                            MM_Composite_Bin(len(pl_module.hparams.config["label_class_count"])))
                setattr(pl_module, f"{split}_{k}_loss", Scalar())
                setattr(pl_module, f"{split}_{k}_feats_show", FeatsShower(f"{version_dir}/feats_show"))
            elif k == "modmis_cls":
                setattr(pl_module, f"{split}_{k}_composite",
                            MM_Composite_Cls(pl_module.hparams.config["label_class_count"]))
                setattr(pl_module, f"{split}_{k}_loss", Scalar())
            elif k == "modmis_reg":
                label_ranges = [torch.arange(0, 2, 1, dtype=torch.float16)] * 4
                label_intervals = [1] * 4
                setattr(pl_module, f"{split}_{k}_composite", MM_Composite_Reg(label_ranges, label_intervals))
                setattr(pl_module, f"{split}_{k}_loss", Scalar())
            elif k == "vqa":
...
def epoch_wrapup(pl_module):
    check_to_save_roc(pl_module, False)
    phase = "train" if pl_module.training else "val"
    the_metric = 0  # 被监视值，checkpoint保存的依据
    ...
    for loss_name, v in pl_module.hparams.config["loss_names"].items():
        ...
        if loss_name in ("modmis_bin", "modmis_cls", "modmis_reg"):
            pl_module.log(
                f"{loss_name}/{phase}/loss_epoch",
                getattr(pl_module, f"{phase}_{loss_name}_loss").compute(),
            )
            getattr(pl_module, f"{phase}_{loss_name}_loss").reset()
            com_scores = getattr(pl_module, f"{phase}_{loss_name}_composite").compute()
            value = com_scores["Auc"]
            for k, v in com_scores.items():
                pl_module.log(f"{loss_name}/{phase}/{k}", v,)
            getattr(pl_module, f"{phase}_{loss_name}_composite").reset()
            if phase in pl_module.hparams.config["feats_show_phase"]:
                # getattr(pl_module, f"{phase}_{loss_name}_feats_show").plot_tsne(f"{phase}_{pl_module.trainer.current_epoch}")  # 画T-SNE
                getattr(pl_module, f"{phase}_{loss_name}_feats_show").plot_field_rela_heatmap(f"{phase}_{pl_module.trainer.current_epoch}")  # 画字段的相关性热力图
                getattr(pl_module, f"{phase}_{loss_name}_feats_show").reset()
        elif loss_name == "vqa":
    ...
    print(phase, "weighted auc:", the_metric)
...
def check_to_save_roc(pl_module, verbose=True):
    phase = "train" if pl_module.training else "val"
    # phase = "test" if pl_module.hparams.config["test_only"] else "test"
    # print(phase, pl_module.hparams.config["test_only"])


    cur_epoch = pl_module.trainer.current_epoch
    save_gap = pl_module.hparams.config["save_roc_every_n_epoch"]
    if cur_epoch % save_gap == 0 or phase == "test":
        for k, v in pl_module.hparams.config["loss_names"].items():
            if v < 1 or not hasattr(pl_module, f"{phase}_{k}_composite"):
                continue
            roc_data = getattr(pl_module, f"{phase}_{k}_composite").draw_roc(show=False)
            auc_list = [torch.mean(roc_data[-1]), *roc_data[-1]]
            best_phase_roc = getattr(pl_module, f"best_{phase}_roc")
            update_flag = False
            if len(best_phase_roc) == 0:
                for item_id in range(len(auc_list)):
                    update_flag = True
                    best_phase_roc.append((cur_epoch, auc_list[item_id], *roc_data))
                    if verbose:
                        print(f"epoch{cur_epoch} {phase} auc{item_id} updated.")
            else:
                for item_id in range(len(auc_list)):
                    if auc_list[item_id] > best_phase_roc[item_id][1]:
                        update_flag = True
                        best_phase_roc[item_id] = (cur_epoch, auc_list[item_id], *roc_data)
                        if verbose:
                            print(f"epoch{cur_epoch} {phase} auc{item_id} updated.")
            if update_flag and verbose:
                print(auc_list)
    if cur_epoch == pl_module.hparams.config["max_epoch"] - 1 or phase == "test":
        for k, v in pl_module.hparams.config["loss_names"].items():
            if v < 1 or not hasattr(pl_module, f"{phase}_{k}_composite"):
                continue
            roc_data_dir = f"{pl_module.hparams.config['log_dir']}/{pl_module.hparams.config['pl_log_name']}/"
            roc_data_dir += f"version_{max([int(item[8:]) for item in os.listdir(roc_data_dir)])}/roc_records"
            os.makedirs(roc_data_dir, exist_ok=True)
            for item_id, item in enumerate(getattr(pl_module, f"best_{phase}_roc")):
                with open(f"{roc_data_dir}/{k}_{phase}_roc{item_id}_{item[0]}.pkl", 'wb') as f:
                    pickle.dump(item, f)
